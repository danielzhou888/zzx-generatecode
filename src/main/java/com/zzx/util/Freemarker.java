...
//			File file = new File(PathUtil.getClasspath() + filePath + outFile);
			File file = new File(PathUtil.getCodeLocation() + filePath + outFile);
...
//            String classResources = PathUtil.getClassResources();
            String classResources = PathUtil.getProjectLocation();
            File file = new File(classResources + "/ftl/" + ftlPath);
			System.out.println("*************************"+classResources);
			System.out.println("*************************"+file.getAbsolutePath());
			cfg.setDirectoryForTemplateLoading(file);		//设定去哪里读取相应的ftl模板文件
